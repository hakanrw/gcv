...
#define LIBGOBJECT "gobject-2.0"
#define LIBGLIB "glib-2.0"
#define LIBGLIB_PROFILE LIBGLIB
		      ...
		      int * in_added_libraries)
{
  ...
  /* The first input file with an extension of .vala.  */
  const char *first_vala_file = NULL;
  ...
  for (i = 1; i < argc; i++)
	...
	case OPT_l:
	  if (strcmp (arg, MATH_LIBRARY) == 0)
	    ...
	    /* Unrecognized libraries (e.g. -lfoo) may require libgobject.  */
	...
	case OPT_x:
	  if (library == 0 && strcmp (arg, "gobject") == 0)
	    library = 1;
	  break;
	...
	case OPT_Wl_:
	  /* Arguments that go directly to the linker might be .o files,
	     or something, and so might cause libglib to be needed.  */
	  if (library == 0)
	    library = 1;
	  break;
...
//	case OPT_static_libgobject:
//	  library = library >= 0 ? 2 : library;
//	  args[i] |= SKIPOPT;
//	  break;
// FIXME: libgobject static linking is not possible yet

	case OPT_SPECIAL_input_file:
	  if (library == 0)
	    library = 1;

      printf("input file: %s\n", arg); // TODO add to vapi extract list

	  if (first_vala_file == NULL)
	    {
	      int len;

	      len = strlen (arg);
	      if (len > 5 && strcmp (arg + len - 5, ".vala") == 0)
            first_vala_file = arg;
	    }
  ...
  /* Copied from gospec. Do we need this? TODO */
  while (i < argc)
    {
      new_decoded_options[j] = decoded_options[i];
  ...
  /* If we didn't see a -o option, add one.  This is because we need
     the driver to pass all .vala files to vala1.  Without a -o option the
     ...
     This approach is also used by Go front end's gospec.
     */
  if (first_vala_file != NULL && !saw_opt_o)
    {
      if (saw_opt_c || saw_opt_S)
	{
	  ...
	  base = lbasename (first_vala_file);
	  baselen = strlen (base) - 5;
	  alen = baselen + 3;
  ...
  /* Add `-lgobject' if we haven't already done so.  */
  if (library > 0)
    {
      generate_option (OPT_l, LIBGOBJECT, 1, CL_DRIVER,
		       &new_decoded_options[j]);
      added_libraries++;
      j++;
...
//#endif
      ...
      generate_option (OPT_l, saw_profile_flag ? LIBGLIB_PROFILE : LIBGLIB, 1,
...
//#endif
      ...
      /* Linking libgobject statically is not supported yet */
      /* TODO */
    }
...
{
  /* Not used for Vala.  TODO */
  return 0;
}

/* Number of extra output files that lang_specific_pre_link may generate.  */
int lang_specific_extra_outfiles = 0; /* Not used for Tiny.  TODO */
